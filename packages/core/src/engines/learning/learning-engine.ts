...
export class LearningEngine {
  ...
  private analyzePattern(event: LearningEvent): void {
    ...
    if (sameType.length >= 3) {
      const existing = this.insights.find((i) => i.pattern === event.type && i.description.includes(event.source))
      if (existing) {
  ...
  summary(): string {
    ...
    if (this.insights.length > 0) {
      ...
      for (const insight of this.insights.slice(0, 5)) {
        lines.push(`  - ${insight.description} (${(insight.confidence * 100).toFixed(0)}% confidence, ${insight.occurrences} occurrences)`)
      }
    }
    return lines.join('\n')
  }
