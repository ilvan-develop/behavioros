...
const createTestDNA = (id: string): DNAPackage => ({
  ...
  governance: [
    { id: `${id}-rule`, name: `Rule ${id}`, level: 'medium', action: 'warn' },
  ],
  quality: [
    { id: `${id}-coverage`, name: `Coverage ${id}`, type: 'test_coverage', threshold: 80 },
  ],
  patterns: [
    { id: `${id}-pattern`, name: `Pattern ${id}`, type: 'collaboration', triggers: ['agent:engineer'], actions: ['code'] },
  ],
})
