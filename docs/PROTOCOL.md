...
Live-verified list (14 files) from a real `init --yes --with-protocol` run:
...
copy. The OpenCode *plugin* (`.opencode/plugins/protocol-enforcer.ts`, real runtime
enforcement beyond the advisory rule file above) also isn't generated yet — see
[GETTING-STARTED.md § OpenCode](GETTING-STARTED.md#opencode) for why and the manual copy
step.
