...
**File:** `.opencode/rules/behavioros-protocol.md` (referenced via `@rules/behavioros-protocol` in `AGENTS.md`)
...
Then create `.opencode/rules/behavioros-protocol.md`:
...
Step 3: Verify protocol is active
        ...
        ↓
Step 4: OpenCode plugin + opencode.json are generated by Step 1 automatically
        (only if opencode.json didn't already exist — see step 5 for verification)
        ↓
...
| `npx @behavioros/cli validate behavioros.yaml` | Validate DNA against schema |
| `npx @behavioros/cli protocol check` | Validate protocol enforcement |
| `npx @behavioros/cli diff --from baseline.yaml --to current.yaml` | Detect behavioral drift |

### Files Created by `init --with-protocol`

Live-verified list (15 files) from a real `init --yes --with-protocol` run:

```
project-root/
├── AGENTS.md                              ← References @rules/behavioros-protocol
├── CLAUDE.md                              ← Claude Code rule file
├── .mcp.json                              ← MCP server config (Claude Code, Cursor, etc.)
├── opencode.json                          ← MCP + plugin config (only if none existed already)
├── .claude/
│   └── settings.json                      ← PreToolUse hook — real enforcement
├── .cursor/
│   ├── hooks.json                         ← beforeShellExecution/beforeMCPExecution — real enforcement
│   └── rules/
│       └── behavioros-protocol.mdc        ← Cursor-specific rule file
├── .windsurf/
│   └── hooks.json                         ← pre_write_code/pre_run_command/pre_mcp_tool_use — real enforcement
├── .windsurfrules                         ← Windsurf rule file
├── .opencode/
│   ├── rules/
│   │   └── behavioros-protocol.md         ← OpenCode-specific rule file
│   └── plugins/
│       └── protocol-enforcer.ts           ← tool.execute.before hook — real enforcement
├── .github/
│   ├── copilot-instructions.md            ← GitHub Copilot rule file
│   └── workflows/
│       └── protocol-compliance.yml        ← CI check
└── scripts/
    ├── validate-protocol.js               ← Shared enforcement script (Claude Code/Cursor/Windsurf)
    ├── validate-dna.js                    ← Cursor afterFileEdit hook
    └── read-agent-state.js                ← Cursor sessionStart hook
```

Note the `docs/PROTOCOL.md` you're reading now is **not** itself generated by `init` — it's
this repo's own copy; a scaffolded project references it by path but doesn't get a local
copy. Every file above is written only if it doesn't already exist — `init --with-protocol`
never overwrites a file you (or another tool) already created; it reports what it skipped.
