  ...
  useEffect(() => {
    async function fetchQuality() {
      try {
        ...
        if (res.ok) {
          const data: QualityResponse = await res.json();
          // Quality metrics from SDK are counts, not gate objects.
          // Gates will be empty until real quality evaluation is run.
          setGates([]);
        }
      } catch {
        // keep empty
          ...
          <div className="space-y-6">
            <div className="grid gap-4 md:grid-cols-3">
              <Card>
                ...
                <CardContent>
                  {loading ? (
                    <div className="h-10 w-20 animate-pulse rounded bg-[#262626]" />
                  ) : (
                    <>
                  ...
                  {loading ? (
                    ...
                    <>
                      <div className="text-4xl font-bold text-yellow-500">
                        {gates.reduce(
                          (acc, gate) =>
                            acc + gate.metrics.filter((m) => m.status !== 'pass').length,
                          0,
                        )}
                      </div>
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                <ClipboardCheck className="h-12 w-12 text-[#262626] mb-4" />
                <p className="text-lg font-medium text-[#fafafa]">No quality gates yet</p>
                <p className="text-sm text-[#a1a1aa] mt-1">Run an audit to see quality metrics.</p>
              </div>
            ) : (
              <div className="grid gap-6 lg:grid-cols-2">
                {gates.map((gate) => (
